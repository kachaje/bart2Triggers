...
BEGIN
  IF new.state = (SELECT program_workflow_state_id FROM program_workflow_state WHERE concept_id = (SELECT concept_id FROM concept_name WHERE name = "Treatment stopped" LIMIT 0,1) AND program_workflow_id = (SELECT program_workflow_id FROM program_workflow WHERE program_id = (SELECT program_id FROM program WHERE concept_id = (SELECT concept_id FROM concept_name WHERE name = "HIV PROGRAM" LIMIT 0,1)))) THEN 
     ...
     INSERT INTO patient_report_details (patient_id, arv_drugs_stopped) VALUES ((SELECT patient_id FROM patient_program WHERE patient_program_id = new.patient_program_id LIMIT 0,1), new.start_date);
  END IF;
  ...
  INSERT INTO patient_report_details (patient_id, latest_state, latest_state_date) VALUES ((SELECT patient_id FROM patient_program WHERE patient_program_id = new.patient_program_id LIMIT 0,1), @state, new.start_date);
